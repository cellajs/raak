...
describe('public read grants — anonymous actor', () => {
  ...
  it('publicSelf denies when publicAt is null or row data is absent', () => {
    expect(getAllDecisions(noPolicies, [], projectSubject(null), { publicGrants: grants }).can.read).toBe(false);

    const noRow: SubjectForPermission = { entityType: 'project', id: 'p1', contextIds: { organization: 'org1' } };
  ...
  it('publicParent grants read from the resolved parent row only', () => {
    expect(getAllDecisions(noPolicies, [], taskSubject(NOW), { publicGrants: grants }).can.read).toBe(true);
    expect(getAllDecisions(noPolicies, [], taskSubject(null), { publicGrants: grants }).can.read).toBe(false);
  ...
  it('publicParentOrSelf grants from either row', () => {
    ...
    expect(getAllDecisions(noPolicies, [], self, { publicGrants: grants }).can.read).toBe(true);
    expect(getAllDecisions(noPolicies, [], parent, { publicGrants: grants }).can.read).toBe(true);
    expect(getAllDecisions(noPolicies, [], neither, { publicGrants: grants }).can.read).toBe(false);
  });

  it('grants read only — other actions stay denied', () => {
    const { can } = getAllDecisions(noPolicies, [], projectSubject(NOW), { publicGrants: grants });
    expect(can.read).toBe(true);
  ...
  it('no grant declared for the entity type → no public read', () => {
    const orgSubject: SubjectForPermission = {
      entityType: 'organization',
      id: 'org1',
      contextIds: {},
      row: { publicAt: NOW },
    };
    expect(getAllDecisions(noPolicies, [], orgSubject, { publicGrants: grants }).can.read).toBe(false);
  });

  it('no publicGrants passed → engine behaves exactly as before', () => {
    expect(getAllDecisions(noPolicies, [], projectSubject(NOW)).can.read).toBe(false);
  });
});

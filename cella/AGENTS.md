...
- **Migrations**: Any fork-breaking upstream change (codemod sweep, schema shift, renamed contract) ships a migration in `cella/migrations/<YYYYMMDDThhmm>-<slug>/` so forks can replay it on their own code after pulling. The UTC-minute timestamp is the stable id and chronological sort key (a date alone collides under high merge activity); the target cella `version` lives in `cella/migrations/manifest.json`, never in the folder name, so folders are never renamed after forks run them. `manifest.json` (one entry per folder: `kind`, `forkBreaking`, `clientCacheBump`, `script`, `roots`, `requires`) is the machine-readable index; `run.ts` diffs it against the fork's applied-set (`cella.migrations.json` at the repo root) to compute the pending plan. **Authoring:** add the folder (from `_TEMPLATE.md`) and the manifest entry in the same PR as the break; a `forkBreaking` change with no migration is treated like a missing `clientCacheVersion` bump. **Applying (forks):** `pnpm exec tsx cella/migrations/run.ts` prints the ordered plan; the `migrate` skill (`cella/skills/migrate/SKILL.md`) drives the apply → `pnpm check` → `run.ts mark <id>` loop. See `cella/migrations/README.md`.

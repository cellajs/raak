...
app.openapi(authGeneralRoutes.invokeToken, async (ctx) => {
  ...
  try {
    ...
    log.info(ctx, 'Token invoked, redirecting with single use token in cookie', {
...
app.openapi(authGeneralRoutes.startImpersonation, async (ctx) => {
  ...
  log.info(ctx, 'Started impersonation', { adminId: adminUser.id, targetUserId });
...
app.openapi(authGeneralRoutes.stopImpersonation, async (ctx) => {
  ...
  log.info(ctx, 'Stopped impersonation', { adminId: adminUserId, targetUserId: session.userId });
...
app.openapi(authGeneralRoutes.resendInvitationWithToken, async (ctx) => {
  ...
  if (oldToken.inactiveMembershipId) {
    ...
    log.info(ctx, 'Membership invitation has been resent', { [entityIdColumnKey]: entity.id });
  } else {
    await mailer.prepareEmails(systemInviteEmail, defaultEmailProps, [recipient], userEmail);
    log.info(ctx, 'System invitation has been resent');
  }
...
app.openapi(authGeneralRoutes.signOut, async (ctx) => {
  ...
  if (confirmMfa) {
    ...
    log.info(ctx, 'User mfa canceled');
  ...
  log.info(ctx, 'User signed out', { userId: currentSession.userId });

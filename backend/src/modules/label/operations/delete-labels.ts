...
import { filterPrimaryLabelDeletes } from '#/modules/label/helpers/primary-labels';
...
export async function deleteLabelsOp(
  ...
  await tenantContextIncludingDeleted(ctx, async (txCtx) => {
    ...
    const allowedIds = await filterPrimaryLabelDeletes(txCtx, permittedIds, rejected);

...
describe('dispatch batch eligibility: fan-out benchmark', () => {
  ...
  // Org members hold read:'own', and every row's createdBy (`author-${id}`) never matches a
  // subscriber's userId (`user-${i}`), so only the org admins (every 10th subscriber, read:1
  // unconditional) are eligible — not the whole hot roster.
  const adminCount = (subscribers: AppStreamSubscriber[]) => subscribers.filter((_, i) => i % 10 === 0).length;
  ...
  const scenarios: Scenario[] = [
    {
      ...
      expectedEligible: adminCount(hot2000),
    },
    {
      name: '40-row batch × 1200 readers',
      subscribers: hot1200,
      makeEvent: () => makeEvent(ORG, 40),
      expectedEligible: adminCount(hot1200),
    },
    {

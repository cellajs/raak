...
describe('deepestAncestorExpr', () => {
  it('raak config: task groups by project, then organization', () => {
    expect(deepestAncestorExpr('task', 't')).toBe('COALESCE(t.project_id, t.organization_id)');
  });

  it('raak config: root context has no grouping expression', () => {

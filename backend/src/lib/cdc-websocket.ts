...
class CdcWebSocketServer {
  ...
  attachToServer(server: ServerType): void {
    ...
    (server as NodeJS.EventEmitter).on('upgrade', (request: IncomingMessage, socket: Duplex, head: Buffer) => {
      ...
      if (env.NODE_ENV === 'production' && !isAllowedCdcSource(remoteIp, request.headers['x-forwarded-for'])) {
        log.warn(null, 'CDC WebSocket rejected disallowed source', {
          ip: remoteIp,
      ...
      if (!env.CDC_SECRET || secret !== env.CDC_SECRET) {
        log.warn(null, 'CDC WebSocket auth failed', {
    ...
    log.info(null, 'CDC WebSocket server attached to HTTP server');
  }
  ...
  private handleConnection(ws: WebSocket): void {
    ...
    if (this.currentConnection) {
      log.info(null, 'Replacing existing CDC Worker connection');
    ...
    log.info(null, 'CDC Worker connected via WebSocket');
    ...
    ws.on('close', (code, reason) => {
      log.info(null, 'CDC Worker disconnected', { code, reason: reason.toString() });
      this.cleanup();
    });

    ws.on('error', (err) => {
      log.error(null, 'CDC WebSocket error', { err });
      this.cleanup();
    });
  }
  ...
  private handleMessage(data: string): void {
    try {
      ...
      if (!result.success) {
        ...
        log.error(null, 'CDC message schema validation failed - message dropped', {
      ...
      if (!isValidEventType(type)) {
        this._parseErrors++;
        log.error(null, 'Unknown event type in CDC message - message dropped', {
          type,
      ...
      log.trace(null, 'CDC message processed', {
      ...
      this._parseErrors++;
      log.error(null, 'Failed to parse CDC message', { err });
    }
  }
  ...
  private handleControlMessage(message: { _control: string; [key: string]: unknown }): void {
    if (message._control === 'catchup_complete') {
      ...
      log.info(null, 'CDC catchup complete — entity caches cleared', {
        eventsProcessed,
        catchupDurationMs,
      });
      return;
    ...
    log.warn(null, 'Unknown CDC control message', { control: message._control });
  }
  ...
  private resetIdleTimer(): void {
    ...
    this.idleTimer = setTimeout(() => {
      log.warn(null, 'CDC WebSocket idle timeout, closing connection');

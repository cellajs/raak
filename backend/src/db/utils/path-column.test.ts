...
describe('pathColumnExpression (SQL shape)', () => {
  it('project-homed product (raak attachment): org id + COALESCE-wrapped project id', () => {
    expect(pathColumnExpression('attachment', false, hierarchy)).toBe(
      `"organization_id"::text || COALESCE('/' || "project_id"::text, '')`,
    );
  });

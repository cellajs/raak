...
// Placing an attachment in a home below the organization reads that channel through the token's mask.
const homeType = hierarchy.getOrderedAncestors('attachment').find((type) => type !== 'organization');
const withHomeRead = (...scopes: string[]) => [...scopes, ...(homeType ? [`${homeType}:read`] : [])].join(' ');
...
describe('MCP on the substrate (Phase E)', async () => {
  ...
  /** The create body's home: the deepest seeded ancestor id below the organization; empty in cella's org-homed default. */
  async function seedAttachmentHome(org: { id: string; tenantId: string }, createdBy: string) {
  ...
  it('showcase 3: a service account creates, reads, renames and deletes through the same tools', async () => {
    const ctx = await serviceToken(withHomeRead('attachment:write'));
  ...
  it('showcase 2: a person consents to a registered app, is refused a rename, steps up and renames as themselves', async () => {
    ...
    const seed = await serviceToken(withHomeRead('attachment:write'));
    ...
    const writer = await userToken(withHomeRead('attachment:read', 'attachment:write'), reader);

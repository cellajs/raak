...
-- Blocks (in order): cdc_setup, counter_functions, immutability_setup, partman_setup, rls_setup, unlogged_setup, publicat_cascade
...
BEGIN
  ...
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'cdc_pub') THEN
      CREATE PUBLICATION cdc_pub FOR TABLE users, organizations, attachments, projects, workspaces, tasks, labels, requests, memberships, inactive_memberships, tenants;
    ...
    ELSE
      ...
      ALTER PUBLICATION cdc_pub SET TABLE users, organizations, attachments, projects, workspaces, tasks, labels, requests, memberships, inactive_memberships, tenants;
...
--> statement-breakpoint
-- ══════════════════════════════════════════════════════════════════════════
-- [publicat_cascade] publicAt cascade — projects → child products
-- ══════════════════════════════════════════════════════════════════════════

...
BEGIN
  IF NEW.tenant_id IS DISTINCT FROM OLD.tenant_id
    ...
    RAISE EXCEPTION 'Cannot modify immutable columns (%) on %', 'tenant_id, organization_id, context_id, context_type, user_id', TG_TABLE_NAME;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  IF NEW.tenant_id IS DISTINCT FROM OLD.tenant_id
       OR NEW.organization_id IS DISTINCT FROM OLD.organization_id
       OR NEW.context_id IS DISTINCT FROM OLD.context_id
       OR NEW.context_type IS DISTINCT FROM OLD.context_type THEN
    RAISE EXCEPTION 'Cannot modify immutable columns (%) on %', 'tenant_id, organization_id, context_id, context_type', TG_TABLE_NAME;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

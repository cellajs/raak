...
-- Postgres, and an UPDATE reading existing columns also applies the tenant
-- SELECT policy, which matches zero rows in a migration session.
...
CREATE UNIQUE INDEX "labels_project_primary_slug_unique" ON "labels" ("project_id","slug") WHERE "mode" = 'primary' AND "deleted_at" IS NULL;--> statement-breakpoint

...
BEGIN
  ...
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'cdc_pub') THEN
      CREATE PUBLICATION cdc_pub FOR TABLE users, organizations, attachments, pages, projects, workspaces, tasks, labels, requests, memberships, inactive_memberships, tenants;
    ...
    ELSE
      ...
      ALTER PUBLICATION cdc_pub SET TABLE users, organizations, attachments, pages, projects, workspaces, tasks, labels, requests, memberships, inactive_memberships, tenants;

...
BEGIN
  IF NEW.tenant_id IS DISTINCT FROM OLD.tenant_id
    ...
    RAISE EXCEPTION 'Cannot modify immutable columns (%) on %', 'tenant_id, context_id, context_type, organization_id, workspace_id, project_id, user_id', TG_TABLE_NAME;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  IF NEW.tenant_id IS DISTINCT FROM OLD.tenant_id
    ...
    RAISE EXCEPTION 'Cannot modify immutable columns (%) on %', 'tenant_id, context_id, context_type, organization_id, workspace_id, project_id', TG_TABLE_NAME;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

...
const cases: Array<{ name: string; parse: () => Record<string, unknown>; defaults: Record<string, unknown> }> = [
  ...
  { name: 'tasks', parse: () => combinedTaskSearchSchema.parse({}), defaults: tasksSearchDefaults },
...
describe('list search defaults', () => {
  ...
  // matchMode is task-only, so it rides outside the shared cases — same drift risk, same guard.
  it('tasks matchMode matches the schema default', () => {
    expect(tasksSearchDefaults.matchMode).toBe(combinedTaskSearchSchema.parse({}).matchMode);
  });
});

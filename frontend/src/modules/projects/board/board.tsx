...
export default function Board() {
  ...
  const handleVerticalArrowKeyDown = async (event: KeyboardEvent) => {
    ...
    if (focusedTaskId) {
      ...
      const { items: tasks } = queryClient.getQueryData(taskKeys.list({ projectId: projects[0].id, orgIdOrSlug: workspace.organizationId })) as {
        items: Task[];
      };
  ...
  const handleTaskOperations = (event: TaskOperationEvent) => {
    ...
    const callback = useMutateTasksQueryData(taskKeys.list({ projectId, orgIdOrSlug: workspace.organizationId }));
    callback(array, action);
    const { items: tasks } = queryClient.getQueryData(taskKeys.list({ projectId, orgIdOrSlug: workspace.organizationId })) as { items: Task[] };

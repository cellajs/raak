 ...
 * (editor -> static description on losing edit focus) cannot collapse and reflow the card. Stores
 * only a pixel height, never content, so it can never show stale text. The height is applied
 * imperatively (no React state) so children never re-render. A hold is released shortly after the
 * content stays empty, so a genuinely empty description settles to its natural size.
 */
